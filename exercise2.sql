...
(1, 'Rahul', 'Sharma', '1968-12-08', 'EmpID1.pic', 'Education includes a BA in psychology from Colorado State University. She also completed (The Art of the Cold Call). Nancy is a member of \'Toastmasters International\'.'),
...
select distinct city, count(city) from customers group by city order by city;

select min(quantity), max(quantity) from order_details;

select sum(quantity), avg(quantity) from order_details;

select productid, productname from products limit 4,11;
...
where o.orderdate >= '2020-01-01' and o.orderdate < '2022-01-01'
